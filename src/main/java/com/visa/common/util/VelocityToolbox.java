...
public class VelocityToolbox {
	...
	public String getRole(Integer roleId) {
		return RoleEnumType.ROLE_MAP.get(roleId) != null ? RoleEnumType.ROLE_MAP
				.get(roleId).getName() : LineRoleEnumType.ROLE_MAP.get(roleId)
				.getName();
	}
	...
	public boolean isAdminRoleId(int roleId) {
		if (RoleEnumType.ADMIN.getId() == roleId
				|| roleId == Constant.SUPER_ADMIN_ROLE_ID) {
			return true;
		}
		return false;
	}
	...
	public boolean isLineAdminRole(int roleId) {
		if (LineRoleEnumType.ADMIN.getId() == roleId
				|| roleId == Constant.SUPER_ADMIN_ROLE_ID) {
			return true;
		}
		return false;
