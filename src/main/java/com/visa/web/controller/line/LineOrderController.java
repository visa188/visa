...
public class LineOrderController {
	 ...
	 */
	@RequestMapping
	public void list(@ModelAttribute(Constant.SESSION_USER) User user,
			Integer page, ModelMap model,
			@ModelAttribute LineOrderSearchBean bean, Integer type,
		...
		if (StringUtils.isEmpty(lineProductOrderSeq)) {
			...
			paraMap.put("startDate", StringUtils.isEmpty(startDate) ? null
					: startDate);
			paraMap.put("endDate", StringUtils.isEmpty(endDate) ? null
					: endDate);
			paraMap.put("orderSeq", StringUtils.isEmpty(orderSeq) ? null
					: orderSeq);
			paraMap.put("alarmOrders",
					alarmOrders == null || alarmOrders == 0 ? null
							: alarmOrders);

			if (LineRoleEnumType.SALESMAN.getId() == user.getRoleId()) {
				paraMap.put("salesmanId", user.getUserId());
			} else if (LineRoleEnumType.SALEMAN_MANAGER.getId() == user
					.getRoleId()) {
			...
			} else if (LineRoleEnumType.SIGNOPERATOR.getId() == user
					.getRoleId()) {
			...
			int[] recordRange = PagingUtil.addPagingSupport(
					Constant.LINE_PAGE_COUNT, recordCount, page,
			...
			for (LineOrder lineOrder : lineOrderDao.selectByPage(paraMap)) {
				if (lineProductOrderSeq.equals(lineOrder
						.getLineProductOrderSeq())) {
	...
	public void add(Integer lineproductId, ModelMap model, int type) {
		if (lineproductId != null) {
			LineProduct product = lineProductDao
					.selectByPrimaryKey(lineproductId);
			model.put("product", product);
		}
		List<Country> countryList = lineCountryDao.selectAllCountry();
		List<LineProduct> lineProductList = lineProductDao
				.selectAllLineProduct();
	...
	public String addSubmit(@ModelAttribute(Constant.SESSION_USER) User user,
		...
		String prefix = StringUtil.paddingZeroToLeft(String.valueOf(orderSeq),
				6);
		lineOrder.setOrderSeq(prefix);
		lineOrder.setSalesmanId(user.getUserId());
		// 散拼团订单时，校验该产品下所有订单客人总量是否大于机位数
		if (lineOrder.getType() == 2) {
			LineProduct product = lineProductDao.selectByPrimaryKey(lineOrder
					.getLineProductId());
			List<LineOrder> lineOrderList = lineOrderDao
					.selectByProductId(lineOrder.getLineProductId());
			int count = 0;
		...
		if (!StringUtils.isEmpty(lineOrder.getSignOperatorId())) {
			lineOrder.setSignOperatorName(userDao.selectByPrimaryKey(
					lineOrder.getSignOperatorId()).getUserName());
		}
	 ...
	 */
	@RequestMapping
	public void edit(@ModelAttribute(Constant.SESSION_USER) User user,
			Integer orderId, Integer currentPage, ModelMap model) {
		LineOrder lineOrder = lineOrderDao.selectByPrimaryKey(orderId);
		List<LinesSrvice> lineServiceList = linesServiceDao
				.selectAllLinesSrvice(orderId);
		...
		List<LineProduct> lineProductList = lineProductDao
				.selectAllLineProduct();
		List<User> signOperList = userDao
				.selectByRoleId(LineRoleEnumType.SIGNOPERATOR.getId());
		...
		List<Country> countryList = lineCountryDao.selectAllCountry();
		model.put("countryList", countryList);
		List<LineNameList> customList = lineNameListDao
				.selectAllLineNameList(orderId);
		model.put("customList", customList);
		Customer customer = customerDao.selectByPrimaryKey(lineOrder
				.getCustomerId());
	...
	public String update(@ModelAttribute(Constant.SESSION_USER) User user,
			LineOrderVo lineOrderVo, Integer currentPage) {
		LineOrder lineOrder = lineOrderDao.selectByPrimaryKey(lineOrderVo
				.getOrderId());
		List<LinesSrvice> tempServiceListDB = linesServiceDao
				.selectAllLinesSrvice(lineOrderVo.getOrderId());
		Map<Integer, LinesSrvice> serviceListDB = VisaUtil
				.dealServiceList(tempServiceListDB);
		Map<Integer, LineNameList> nameListDB = VisaUtil
				.dealNameList(lineNameListDao.selectAllLineNameList(lineOrderVo
						.getOrderId()));

		lineOrderVo.setSalesmanName(userDao.selectByPrimaryKey(
				lineOrderVo.getSalesmanId()).getUserName());
		...
		if (user.getRoleId() == LineRoleEnumType.OPERATOR.getId()
				|| user.getRoleId() == LineRoleEnumType.OPERATOR_MANAGER
						.getId()) {
			...
			for (LinesSrvice srvice : lineOrderVo.getLineOrderService()) {
				if (srvice.getServiceType() == LineSrviceEnumType.LD.getId()
						|| srvice.getServiceType() == LineSrviceEnumType.JP
								.getId()
						|| srvice.getServiceType() == LineSrviceEnumType.DJBS
								.getId()
						|| srvice.getServiceType() == LineSrviceEnumType.DJSJDY
								.getId()
						|| srvice.getServiceType() == LineSrviceEnumType.BX
								.getId()
						|| srvice.getServiceType() == LineSrviceEnumType.QT
								.getId()) {
					dealService(tempServiceListDB, srvice);
				}
			}
		} else if (user.getRoleId() == LineRoleEnumType.VISAOPER.getId()
				|| user.getRoleId() == LineRoleEnumType.VISAOPER_MANAGER
						.getId()) {
			List<Integer> serviceTypeList = new ArrayList<Integer>();
			...
			for (LinesSrvice srvice : lineOrderVo.getLineOrderService()) {
				if (srvice.getServiceType() == LineSrviceEnumType.QZ.getId()
						|| srvice.getServiceType() == LineSrviceEnumType.GGBZ
								.getId()) {
					dealService(tempServiceListDB, srvice);
				}
			}
		...
		// 记录操作日志
		String log = StringUtil.generateUpdateOperLog(lineOrderVo, lineOrder,
		...
		return "redirect:list.do?page=" + currentPage + "&type="
				+ lineOrderVo.getType();
	}
	 ...
	 */
	@RequestMapping
	public String delete(@ModelAttribute(Constant.SESSION_USER) User user,
			int orderId, Integer page) {
		LineOrder lineOrder = lineOrderDao.selectByPrimaryKey(orderId);
	...
	public void export(ModelMap model) {
		model.put("salesmanList",
				userDao.selectByRoleId(LineRoleEnumType.SALESMAN.getId()));
	...
	public void exportSubmit(HttpServletRequest request,
		...
		try {
			...
			this.exportOrderData(toClient, year, month, salesman, yfhkStatus,
					yshkStatus, customerId, company, operatorId);
	 ...
	 *            out
	 */
	private void exportOrderData(OutputStream out, String year, String month,
			String salesmanId, String yfhkStatus, String yshkStatus,
			String customerId, String company, String operatorId) {
		String[] titles = { "订单序号", "销售员", "产品名称", "订单类型", "线路国家", "下单日期",
				"客人数量", "操作员", "送签员", "组团社", "联系人", "联系方式", "团费单价", "总计应收",
				"总计应付", "毛利", "备注" };
		...
		paraMap.put("salesmanId", StringUtils.isEmpty(salesmanId) ? null
				: salesmanId);

		paraMap.put("customerId", StringUtils.isEmpty(customerId) ? null
				: customerId);
		paraMap.put("company", StringUtils.isEmpty(company) ? null : company);
		paraMap.put("operatorId", StringUtils.isEmpty(operatorId) ? null
				: operatorId);

		paraMap.put("yfhkStatus", StringUtils.isEmpty(yfhkStatus) ? null
				: yfhkStatus);
		paraMap.put("yshkStatus", StringUtils.isEmpty(yshkStatus) ? null
				: yshkStatus);
		...
		if (ordersList != null && ordersList.size() > 0) {
			...
			for (; i < ordersList.size(); i++) {
				...
				headerCell.setCellValue(DateUtil.format(p.getOrderDate(),
