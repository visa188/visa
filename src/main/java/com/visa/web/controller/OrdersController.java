...
public class OrdersController {
     ...
     */
    @RequestMapping
    public void list(@ModelAttribute(Constant.SESSION_USER) User user, int page, ModelMap model,
    ...
    public String addSubmit(@ModelAttribute(Constant.SESSION_USER) User user,
        ...
        if (RoleEnumType.SALESMAN.getId() == user.getRoleId()) {
            ...
            // .multiply(new BigDecimal(orders.getNameListSize())).add(orders.getPriceQtzc()));
            // 总计应收款=应收单价*客人数量+其他应收款
            // orders.setPriceZjys(orders.getPriceYsdj()
            // .multiply(new BigDecimal(orders.getNameListSize())).add(orders.getPriceQtys()));
            // 毛利=总计应收-其他支出款
    ...
    public void edit(@ModelAttribute(Constant.SESSION_USER) User user, int orderId, ModelMap model,
        ...
        model.put("nameList", orders.getNameList() == null ? "" : orders.getNameList().split(","));
    ...
    private void exportOrderData(OutputStream out, String year, String month, String salesmanId,
            String yfhkStatus, String yshkStatus) {
