...
public class LineOrderVo extends LineOrder {
	...
	public List<LinesSrvice> getLineOrderService() {
		...
		if (ld != null && ld.startsWith("1")) {
			LinesSrvice linesSrvice = new LinesSrvice(orderId,
					LineSrviceEnumType.LD.getId());
		...
		LinesSrvice bxSrvice = new LinesSrvice(orderId,
				LineSrviceEnumType.BX.getId());
	...
	public List<LineNameList> getLineCustomList() {
		...
		if (nameList != null) {
			...
			for (String customStr : customArray) {
				...
				nameListPo
						.setAgeType(Integer.valueOf(items[2].replace("#", "")));
